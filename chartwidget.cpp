 ...
 * 文件作用: 通用图表组件实现文件
 * 功能描述:
 * 1. 封装 QCustomPlot 基础功能，提供统一的图表展示界面。
 * 2. 支持单坐标系 (Mode_Single) 和双坐标系堆叠 (Mode_Stacked) 模式。
 * 3. [优化] 在堆叠模式下，消除上下坐标系间距，实现无缝拼接。
 * 4. [优化] 上方压力坐标系隐藏横轴数值，共用下方产量坐标系的时间轴。
 * 5. [新增] 支持开/关井事件线（红/绿虚线），在双坐标模式下贯穿显示（从底至顶）。
 * 6. 实现鼠标交互：缩放、拖拽、移动数据、编辑标注、右键菜单等。
 * 7. [修复] 修正 QCPItemLine 坐标轴设置方式，解决编译错误。
 */
...
void ChartWidget::addEventLine(double x, int type) {
    ...
    auto createLine = [&](QCPAxisRect* rect) -> QCPItemLine* {
        ...
        // [修复] QCPItemLine 没有 setAxes，需要分别设置 start 和 end 的轴
        // 这一步至关重要，特别是当使用 ptPlotCoords 时，它需要知道参考哪个轴
    ...
    if (m_chartMode == Mode_Stacked && m_topRect && m_bottomRect) {
        ...
        // 由于布局中消除了间隙，上方的底边与下方的顶边重合
        // 上方线段(Top 0-1) 和 下方线段(Bottom 0-1) 将在视觉上完美连接，形成贯穿线
...
void ChartWidget::on_btnDrawLine_clicked() {
    m_lineMenu->exec(ui->btnDrawLine->mapToGlobal(QPoint(0, ui->btnDrawLine->height())));
}
...
void ChartWidget::onPlotMouseMove(QMouseEvent* event) {
    if (event->buttons() & Qt::LeftButton) {
        ...
        if ((m_interMode == Mode_Moving_Data_X || m_interMode == Mode_Moving_Data_Y) && m_movingGraph) {
            ...
            // [同步移动开/关井线]
            // 如果是在堆叠模式下移动产量曲线(bottomRect)，则同步移动事件线
